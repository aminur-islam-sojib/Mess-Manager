...
export default function UserSettingsClient({
  ...
  const pendingRequests = data.depositRequests.filter(
        ...
        <div className="mt-2 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-foreground">
              Manage your account at {data.mess.messName}
            </h1>
            <p className="mt-1 text-sm text-muted-foreground">
              Update your profile, review mess details, manage deposit requests,
              and control your personal meal preferences.
            </p>
          </div>
          <div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
            <SummaryStat
              label="Members"
              value={String(data.mess.totalMembers)}
            />
            <SummaryStat
              label="Joined"
              value={new Date(data.mess.joinedDate).toLocaleDateString(
                "en-US",
                {
                  month: "short",
                  day: "numeric",
            ...
            />
            <SummaryStat label="Pending" value={String(pendingRequests)} />
          </div>
        </div>
      </div>
